...
class Agent(common.Module):
  ...
  def train(self, data, state=None):
    ...
    metrics.update(mets)
    start = outputs['post']
    start['action'] = data['action']
    start['reward'] = data['reward']
#    reward = lambda seq: self.wm.heads['reward'](seq['feat']).mode()
    reward = lambda feat: self.wm.heads['reward'](feat).mode()
...
class WorldModel(common.Module):
  ...
  def imagine(self, policy, start, is_terminal, horizon, reward_fn):
...
#    start['action'] = tf.zeros_like(policy(start['feat']).mode())
    ...
    for _ in range(horizon):
      ...
      for key, value in {**state, 'action': action, 'feat': feat, 'reward': reward}.items():
...
class ActorCritic(common.Module):
  ...
  def __init__(self, config, act_space, tfstep):
    ...
    self.critics = [common.MLP([], **self.config.critic) 
                                    for _ in range(self.config.num_critics)]
    if self.config.slow_target:
      self.target_critics = [common.MLP([], **self.config.critic) 
                                      for _ in range(self.config.num_critics)]
  ...
  def train(self, world_model, start, is_terminal, reward_fn):
    ...
    with tf.GradientTape(persistent=True) as actor_critic_tape:
      seq = world_model.imagine(self.actor, start, is_terminal, hor, reward_fn)
#      reward = reward_fn(seq)
      seq['reward'], mets1 = self.rewnorm(seq['reward'])
      metrics.update({f'reward_{k}': v for k, v in mets1.items()})
      
      reward = tf.cast(seq['reward'], tf.float32)
      disc = tf.cast(seq['discount'], tf.float32)
      critics = [critic(tf.concat([seq['feat'][:-1], 
                                  seq['action'][1:]], 
                                  axis = -1)) for critic in self.critics]
      values = [critic.mode() for critic in critics]
      #TODO verify optimism/pessimism
      value = tf.math.reduce_max(tf.stack(values), axis = 0)
      
      # Skipping last time step because it is used for bootstrapping.
      MCReturn = common.lambda_return(
        reward[1:-1], value[:-1], disc[1:-1],
        bootstrap=value[-1],
        lambda_=self.config.discount_lambda,
        axis=0)
      
      metrics.update({f'critic_{i}': value.mean() for i,value in enumerate(values)})
      metrics.update({f'critic__real_data_{i}': value[0].mean() for i,value in enumerate(values)})
      metrics.update({f'critic_model_data_{i}': value[1:].mean() for i,value in enumerate(values)})
      metrics['critic'] = value.mean()
      metrics['MCReturn'] = MCReturn.mean()
    
      policy = self.actor(tf.stop_gradient(seq['feat'][:-2]))
      objective = (1.0 - self.config.discount_lambda) * value[:-1] + self.config.discount_lambda  * MCReturn
      ent = policy.entropy()
      ...
      #Critics loss
      critics = [critic(tf.concat([seq['feat'][:-2], 
                                  seq['action'][1:-1]], 
                                  axis = -1)) for critic in self.critics]
      target_values = [critic(tf.concat([seq['feat'][:-1], 
                                         seq['action'][1:]], axis = -1)).mode() 
                                            for critic in self.target_critics]
      #target_value = tf.math.reduce_min(tf.stack(target_values), axis = 0)
      ...
      target_MCReturn = common.lambda_return(
        reward[1:-1], target_value[:-1], disc[1:-1],
      ...
      critic_losses_model_data = [-(critic.log_prob(target) * weight).mean() for critic in critics]
      critic_loss_model_data = tf.reduce_mean(critic_losses_model_data)
      metrics.update({f'critic_loss_model_data_{i}':l for i,l in enumerate(critic_losses_model_data)})
      metrics['critic_loss_model_data'] = critic_loss_model_data
      
      data_critics = [critic(tf.concat([seq['feat'][0, :, :-2], 
                                        seq['action'][0, :, 1:-1]], 
                                        axis = -1)) for critic in self.critics]
      
      data_target =  seq['reward'][0, :, 1:-1] + \
      disc[0, :, 1:-1] * ((1.0 - self.config.discount_lambda) * target_value[0, :, 1:-1] + \
                           self.config.discount_lambda * target_MCReturn[0, :, 1:-1])

      critic_losses_real_data = [-(critic.log_prob(data_target)).mean() for critic in data_critics]
      critic_loss_real_data = tf.reduce_mean(critic_losses_real_data)
      metrics.update({f'critic_loss_real_data_{i}':l for i,l in enumerate(critic_losses_real_data)})
    ...
    metrics.update(self.actor_opt(actor_critic_tape, actor_loss, self.actor))
    metrics.update(self.critic_opt(actor_critic_tape, critic_loss, self.critics))
